...
object Option {
  ...
  def map2[A, B, C](a: Option[A], b: Option[B])(f: (A, B) => C): Option[C] = a flatMap {a_value => b map { f(a_value, _) } }

  def sequence[A](a: List[Option[A]]): Option[List[A]] = a.foldRight[Option[List[A]]](Some(List())) {(x, l) =>
    map2(x, l) {_ :: _}
  }

  def traverse[A, B](a: List[A])(f: A => Option[B]): Option[List[B]] = a.foldRight[Option[List[B]]](Some(List())) {(x, l) =>
    map2(f(x), l) {_ :: _}
  }
